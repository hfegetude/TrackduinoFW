...
static void errorLed(void){
	DDRD|=2;//make sure led is output
...
static void captureImgOV7670(uint16_t wg,uint16_t hg){
  ...
  //Wait for vsync it is on pin 3 (counting from 0) portD
   while(!(PINE& (1<<5)     ));//wait for high
   while((PINE&  (1 << 5)   ));//wait for low
  ...
  while(hg--){
    ...
    lg2=wg;
    while(lg2--){
      while((PINE&(1<< 4)));//wait for low
      *b++= PINC;
      while(!(PINE&(1 << 4)));//wait for high
    }
